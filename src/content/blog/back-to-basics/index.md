...
For [my website](https://ddimmery.com/posts/quarto-website/), I decided I didn't want to manually write out the metadata for my publications anymore. So in one afternoon, I vibecoded [a system](https://github.com/ddimmery/quarto-website/blob/main/research.qmd) which connects to the [Semantic Scholar API](https://www.semanticscholar.org/product/api)[^fn7], checks for new papers on my profile and if it finds them, sets up the metadata in [`papers.yaml`](https://github.com/ddimmery/quarto-website/blob/main/papers.yaml). It doesn't display until I flip a switch from `visible: false` (the default) to `visible: true`. Thus, I'm not too worried about the system going haywire: nothing is posted without me getting the chance to review it, but it reduces tedious data entry. The technology is there to vastly reduce the time between an idea and a prototype of that idea. For this Semantic Scholar system, it took me just a few minutes to set Claude up with instructions, and I had a complete and functional prototype no more than one hour later, most of which did not require me to be thinking about the implementation (verified via [commit log](https://github.com/ddimmery/quarto-website/commits/main/?since=2025-08-05&until=2025-08-05)).

[^fn7]: I am increasingly a fan of the work Semantic Scholar is doing as a more palatable alternative to Google Scholar. Unfortunately, they are probably doomed to failure because Google Scholar uses an expansive definition of citation that means their numbers are bigger. Even as a fan of Semantic Scholar, I definitely provide the Google Scholar citation count and h-index for annual reviews.
